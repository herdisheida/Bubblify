...
export default function CartPage() {
  ...
  return (
    <div className="p-10">
      ...
      {cart.map((item) => (
        <div key={item.bubble.id} className="flex justify-between border p-4 my-4 rounded">
          ...
          <div className="flex gap-3 items-center">
            ...
            <button onClick={() => decreaseQuantity(item.bubble.id)} className="font-bold cursor-pointer hover:text-violet-500"> - </button>
            <span>{item.quantity}</span>
            <button onClick={() => increaseQuantity(item.bubble.id)} className="font-bold cursor-pointer hover:text-violet-500"> + </button>
            
            {/* trash icon */}
            <button onClick={() => removeFromCart(item.bubble.id)} className="font-bold cursor-pointer hover:text-violet-500">
      ...
      {cart.length > 0 && (
        <button className="bg-sky-500 hover:bg-sky-700 text-white px-6 py-3 rounded-xl mt-4 transition-colors      cursor-pointer hover:scale-101 hover:shadow-md"
