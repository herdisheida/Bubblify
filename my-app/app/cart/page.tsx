      ...
      {cart.map((item) => (
        <div key={item.bubble.id} className="flex justify-between border-b py-4">
          ...
          <div className="flex gap-3 items-center">
            <button onClick={() => decreaseQuantity(item.bubble.id)} className="font-bold"> - </button>
            <span>{item.quantity}</span>
            <button onClick={() => increaseQuantity(item.bubble.id)} className="font-bold"> + </button>
            <button onClick={() => removeFromCart(item.bubble.id)}>
